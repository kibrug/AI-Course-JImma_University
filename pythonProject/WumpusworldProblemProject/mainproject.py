...
def bfs(current, target):  #### used for going from one safe path to another
    ...
    while q:
        ...
        for i in range(4):
            ...
            if legal(newr, newc) and current_status[newr][newc] == 1 and visited[newr][
                newc] == False:  # checking if next move is legal
    ...
    for i in range(4):
        for j in range(4):
            for l, r in (('B', 'P'), ('S', 'W')):
                ...
                for x, y in neighbours:  #
                    kb.append({(f'{r}{x}{y}', 0),
                               (f'{l}{i + 1}{j + 1}', 1)})  ## this expression corresponds to (pV~r)and(pV~q) and so on
                if (r == 'P'):
                ...
                for j1 in range(4):
                    if i1 == i and j1 == j:
                        ...
                        if (temp1 not in store) or (temp2 not in store):
                            ...
                            k += 1
                            kb.append(
                                {(f'W{i + 1}{j + 1}', 0), (f'W{i1 + 1}{j1 + 1}', 0)})  # there are atmost one wumpus
...
def dpll(expr):
    ...
    polarity, uc = unit_clauses(
        expr_new)  # getting unit clauses and also checking if they are any polarising unit clauses
    ...
    if len(uc) != 0:
        ...
        for i in uc:
            ...
            # print("new one is :")
            # print(new_one)
            to_change = [c.difference(new_one) for c in
                         to_change]  # remove the negation of the unit clauses from the clauses
